...
results = [
    {
         "folder": "./paper/experiments/{}/16_5".format(name),
         "label": "ICC n=16 f=5",
         "N": 16,
    },
    {
         "folder": "./paper/experiments/{}/16_5_0".format(name),
         "label": "FICC n=16 f=5 p=0",
         "N": 16,
    },
    # {
    #      "folder": "./paper/experiments/{}/16_3_3".format(name),
...
for res in results:
    ...
    for filename in os.listdir(res["folder"]):
        ...
        if filename.endswith(".json"):
            ...
            location_latencies = [value["latency"]["secs"] + value["latency"]["nanos"] / 1000000000 for value in list(data['finalization_times'].values())]
            if proposer_only:
    ...
    print(name, res["label"], "blocks finalized:", min)
...
for res in results:
    ...
    for filename in os.listdir(res["folder"]):
        if filename.endswith(".json"):
            ...
            location_latencies = [value["latency"]["secs"] + value["latency"]["nanos"] / 1000000000 for value in list(data['finalization_times'].values())][5:]
            if proposer_only:
                location_latencies = location_latencies[((1-n_replica) % res["N"])::res["N"]]
            else:
                location_latencies = [loc for loc in location_latencies if loc != 0]
    ...
    print("stats ",res["label"], statistics.mean(subnet_latencies), statistics.stdev(subnet_latencies))
